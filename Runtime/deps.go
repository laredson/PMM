...
func ensureDependencies(root string, ifNeeded, refreshMappings bool) (DependencyStatus, error) {
	...
	repak := filepath.Join(root, "Tools", "repak.exe")
	if existsFile(repak) && !hashMatches(repak, m.RepakSha256) {
		_ = os.Remove(repak)
	}
	if !hashMatches(repak, m.RepakSha256) {
		if near := findNearbyVerified(root, "Tools/repak.exe", m.RepakSha256); near != "" {
	...
	mapping := filepath.Join(root, "Mappings", "Mappings.usmap")
	mappingGood := hashMatches(mapping, m.MappingsSha256) && fileSize(mapping) > 1024
	if refreshMappings || !mappingGood {
		if !refreshMappings {
			if near := findNearbyVerified(root, "Mappings/Mappings.usmap", m.MappingsSha256); near != "" && fileSize(near) > 1024 {
	...
	runtimeDir := filepath.Join(root, "Tools", "dotnet", m.DotnetRuntimeContract)
...
func inspectDependencies(root string, m ReleaseManifest) DependencyStatus {
	...
	s.RepakOK = hashMatches(filepath.Join(root, "Tools", "repak.exe"), m.RepakSha256)
	s.MappingsOK = fileSize(filepath.Join(root, "Mappings", "Mappings.usmap")) > 1024 && hashMatches(filepath.Join(root, "Mappings", "Mappings.usmap"), m.MappingsSha256)
	...
	if s.DotnetOK && s.ManagedPayloadOK {
		core := filepath.Join(root, "Tools", "PMMCore", "bin", "pmmcore.dll")
		...
		reader := filepath.Join(root, "Tools", "AssetReader", "bin", "PMM.AssetReader.dll")
...
func repairOodle(root string, m ReleaseManifest) bool {
	p := filepath.Join(root, "Tools", "oo2core_9_win64.dll")
...
func saveDotnetSelection(root string, m ReleaseManifest, dotnet string) {
	marker := filepath.Join(root, "Tools", "dotnet-host.txt")
