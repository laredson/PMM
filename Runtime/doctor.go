...
func runtimeDoctor(root string) RuntimeDoctor {
	...
	req := []string{"PMM.exe", "PMMRuntime.exe", "Knowledge/package-rules.json", "Tools/repak.exe", "Tools/PMMCore/bin/pmmcore.dll", "Tools/AssetReader/bin/PMM.AssetReader.dll", "Mappings/Mappings.usmap"}
	files := map[string]bool{}
	for _, r := range req {
		p := filepath.Join(root, filepath.FromSlash(r))
		files[r] = existsFile(p)
	}
	return RuntimeDoctor{Protocol: "PMM_RUNTIME_DOCTOR_V1", RuntimeVersion: runtimeVersion, BuildID: readTrim(filepath.Join(root, "BUILD_ID.txt")), Root: root, Security: runtimeSecurity(root), Dependencies: deps, Knowledge: validateKnowledge(root), Game: detectPalworld(root), RequiredFiles: files}
}
