...
func main() {
	...
	if args[0] == "doctor" {
		...
		if !d.RequiredFiles["PMMRuntime.exe"] || !d.RequiredFiles["Runner/routes.json"] {
			os.Exit(12)
		}
		return
	}
...
func newHost(root string) (*Host, error) {
	...
	sdir := filepath.Join(root, "Data", "HostSessions", sid)
	...
	return &Host{Root: root, SessionID: sid, SessionDir: sdir, HostLog: filepath.Join(root, "Logs", "PMMHost.log"), OutLog: filepath.Join(sdir, "runner.stdout.log"), ErrLog: filepath.Join(sdir, "runner.stderr.log"), StateFile: filepath.Join(sdir, "state.txt")}, nil
}
...
func (h *Host) resolveChild(operation string, args []string, sec SecurityStatus) (ChildPlan, error) {
	routesPath := filepath.Join(h.Root, "Runner", "routes.json")
	...
	runner := filepath.Join(h.Root, "Runner", "PMM-Runner.ps1")
...
func (h *Host) createHandoff(reason string, code int, message string) (string, error) {
	outDir := filepath.Join(h.Root, "AI_HANDOFFS")
	...
	zw := zip.NewWriter(f)
	defer zw.Close()
	incident := map[string]any{"protocol": "PMM_AI_HANDOFF_EMERGENCY_V1", "reason": reason, "exit_code": code, "message": message, "session_id": h.SessionID, "build_id": strings.TrimSpace(readSmall(filepath.Join(h.Root, "BUILD_ID.txt"), 4096)), "host_version": hostVersion, "created_utc": time.Now().UTC().Format(time.RFC3339Nano)}
	...
	candidates := []struct {
		...
		{filepath.Join(h.Root, "Logs", "PalModMerger.log"), "logs/PalModMerger.log", 1024 * 1024},
		{filepath.Join(h.Root, "BUILD_ID.txt"), "pmminfo/BUILD_ID.txt", 0},
		{filepath.Join(h.Root, "VERSION.txt"), "pmminfo/VERSION.txt", 0},
		{filepath.Join(h.Root, "RELEASE_MANIFEST.json"), "pmminfo/RELEASE_MANIFEST.json", 0},
	}
...
func doctor(root string) Doctor {
	req := []string{"PMMRuntime.exe", "Runner/routes.json", "Runner/PMM-Runner.ps1", "Setup-Dependencies.ps1", "Start-PalModMerger.ps1", "Knowledge/package-rules.json", "Tools/repak.exe", "Tools/PMMCore/bin/pmmcore.dll"}
	...
	return Doctor{Protocol: "PMM_DOCTOR_V1", HostVersion: hostVersion, BuildID: strings.TrimSpace(readSmall(filepath.Join(root, "BUILD_ID.txt"), 4096)), Root: root, Security: securityStatus(root), RequiredFiles: files, RequiredHashes: hashes}
}
func securityStatus(root string) SecurityStatus {
	...
	note := "PMM.exe does not change or bypass Windows application-control policy. RC1 can dispatch the normal start route directly to PMMRuntime.exe without starting PowerShell. Script routes remain external and optional."
