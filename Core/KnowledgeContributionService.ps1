...
function Get-PMMKnowledgeContributionRoot {
  $root=Join-Path $Script:Root 'Data\KnowledgeContributions'
...
function Find-PMMReviewFolderByCaseId([string]$CaseId) {
  $reviewRoot=Join-Path $Script:Root 'Data\Review'
...
function New-PMMReconstructedSolutionZip([string]$SolutionRoot,[string]$OutputZip) {
  $stage=Join-Path $Script:Root ('Cache\ContributionSolution_'+[guid]::NewGuid().ToString('N'))
  try{
...
function Export-PMMKnowledgeContribution([string]$CaseId,[string]$RuntimeNotes='') {
  ...
  $solutionRoot=Join-Path (Join-Path $Script:Root 'Data\ManualSolutions') $CaseId
  ...
  if(-not$review){throw 'The original review case is unavailable. Keep Data\Review until a tested contribution is exported.'}
  ...
  $stage=Join-Path $Script:Root ('Cache\KnowledgeContribution_'+[guid]::NewGuid().ToString('N'))
  ...
  try{
    ...
    $handoff=@(Get-ChildItem -LiteralPath $review -Filter ('AI_HANDOFF_'+$CaseId+'*.zip') -File -ErrorAction SilentlyContinue|Select-Object -First 1)
    if($handoff.Count -gt 0){Copy-Item -LiteralPath $handoff[0].FullName -Destination (Join-Path $stage 'original-handoff.zip') -Force}
    ...
    $versionPath=Join-Path $Script:Root 'VERSION.txt';$pmmVersion=if(Test-Path -LiteralPath $versionPath -PathType Leaf){(Get-Content -LiteralPath $versionPath -Raw).Trim()}else{'unknown'}
    $runtime=[ordered]@{
    ...
    $contribution=[ordered]@{
      ...
      OriginalHandoffIncluded=($handoff.Count -gt 0);OriginalHandoffSha256=$(if($handoff.Count -gt 0){Get-Sha256 (Join-Path $stage 'original-handoff.zip')}else{''});
