...
against Vanilla, applies non-authoritative behavior hints from Knowledge/, and
...
function Get-PMMKnowledgeSummary {
  ...
  $behaviorsPath=Join-Path $Script:Root 'Knowledge\known-behaviors.json'
  ...
  $fixturesPath=Join-Path $Script:Root 'Knowledge\known-fixtures.json'
...
function Get-PMMSemanticHintRules {
  $path=Join-Path $Script:Root 'Knowledge\behavior-symbols.json'
...
function Get-PMMKnownSemanticFixture($Group,[array]$ProviderRecords) {
  $path=Join-Path $Script:Root 'Knowledge\known-fixtures.json'
...
function Get-PMMKnownBehaviorContext($Group,[array]$ProviderRecords) {
  $path=Join-Path $Script:Root 'Knowledge\known-behaviors.json'
  if(-not(Test-Path -LiteralPath $path -PathType Leaf)){return @()}
