...
`package-rules.json` stores conservative installer/package knowledge that must be applied before byte-level asset merging. These rules are not merge recipes: they describe mutually exclusive source PAK configurations declared by a package author. PMM keeps every imported PAK in the library, asks for an explicit package choice when an invalid simultaneous configuration is detected, then excludes the unselected alternative(s) from the next Analyze pass and Deploy.

The first `PMM_PACKAGE_RULES_V1` rule covers Nexus Palworld mod #4935: use `pelt1-luny_P.pak` alone, or use `pelt1-ribunny_P.pak` together with `pelt1-ribunny-shine_P.pak`.
