...
Core responsibilities live in Core/*.ps1.  The UI is responsible only for:
...
Palworld Manager Merger v1.1 keeps the v1.0 proven merge adapters and adds the first runtime-proven community Knowledge production recipe plus safer duplicate-row DataTable semantics. It does not use whole-asset fallback for Unreal asset families. Analyze
...
# ---------------------------------------------------------------------------
. "$Script:Root\Core\Common.ps1"
. "$Script:Root\Core\GameLocator.ps1"
. "$Script:Root\Core\PakService.ps1"
. "$Script:Root\Core\LibraryService.ps1"
. "$Script:Root\Core\SaveService.ps1"
. "$Script:Root\Core\SemanticLab.ps1"
. "$Script:Root\Core\GameReferenceService.ps1"
. "$Script:Root\Core\KnowledgeRecipeService.ps1"
. "$Script:Root\Core\MergeEngine.ps1"
. "$Script:Root\Core\KnowledgeContributionService.ps1"
...
$xamlPath = Join-Path $Script:Root ("UI\MainWindow.{0}.xaml" -f $lang)
try {
...
# ---------------------------------------------------------------------------
# Background Analyze / Build operations.
...
function Start-PMMBackgroundOperation {
  ...
  $worker=Join-Path $Script:Root 'Core\OperationWorker.ps1'
  if(-not(Test-Path -LiteralPath $worker -PathType Leaf)){throw 'Core\OperationWorker.ps1 is missing.'}

  $jobsRoot=Join-Path $Script:Root 'Cache\OperationJobs'
  ...
  try{
    ...
    if($Operation -eq 'Analyze'){Set-PMMAnalyzeBusy $false}else{Set-PMMBuildBusy $false}
    $Script:BackgroundOperationKind=''
...
function Start-PMMGameReferenceBuild {
  ...
  $worker=Join-Path $Script:Root 'Core\GameReferenceWorker.ps1'
  if(-not(Test-Path -LiteralPath $worker -PathType Leaf)){throw 'Core\GameReferenceWorker.ps1 is missing.'}

  $jobsRoot=Join-Path $Script:Root 'Cache\GameReferenceJobs'
  New-Item -ItemType Directory -Force -Path $jobsRoot|Out-Null
  $job=Join-Path $jobsRoot ([guid]::NewGuid().ToString('N'))
  New-Item -ItemType Directory -Force -Path $job|Out-Null
...
$Script:CurrentUnsupportedAssetKey = ''
    ...
    $Script:TxtConflictAsset.Text = ((L 'File: ' 'Archivo: ') + [string]$AssetSummary.Asset + $(if($reason){"`n" + (L 'Reason: ' 'Motivo: ') + $reason}else{''}))
...
function Show-SelectedUnsupportedAsset {
  ...
  $Script:BtnOpenAIHandoff.Tag=[string]$item.AIHandoff
  $Script:BtnOpenAIHandoff.IsEnabled=(-not[string]::IsNullOrWhiteSpace([string]$item.AIHandoff) -and (Test-Path -LiteralPath ([string]$item.AIHandoff) -PathType Leaf))
  $Script:BtnImportManualSolution.Tag=[string]$item.ReviewFolder
  $Script:BtnImportManualSolution.IsEnabled=(-not[string]::IsNullOrWhiteSpace([string]$item.ReviewFolder) -and (Test-Path -LiteralPath (Join-Path ([string]$item.ReviewFolder) 'case.json') -PathType Leaf))
...
function Refresh-PMMAnalysisWorkspace {
  ...
  $participants=@($assets|Where-Object{[string]$_.Mode -notin @('Identical','Unsupported')}|ForEach-Object{@($_.Providers)}|ForEach-Object{[string]$_}|Where-Object{$_}|Sort-Object -Unique)
...
function Update-BuildButtonState {
  ...
  $unsupported=@(Get-PMMUnsupportedAssets)
  $alreadyPatched=($plan.PSObject.Properties.Name -contains 'AlreadyPatched' -and [bool]$plan.AlreadyPatched)
  $requiresPatch=$alreadyPatched
  if($plan.PSObject.Properties.Name -contains 'Assets'){$requiresPatch=(@($plan.Assets|Where-Object{$_.Mode -ne 'Identical'}).Count -gt 0)}
  ...
  $Script:BtnBuild.IsEnabled=(-not$alreadyPatched -and $requiresPatch -and $unsupported.Count -eq 0 -and $unresolved -eq 0 -and -not$currentDecisionPatch)
  $Script:BtnDeploy.IsEnabled=($noPatchSelected -or -not$requiresPatch -or $null -ne $selectedPatch)
  ...
  if($unsupported.Count -gt 0){
    if($noPatchSelected){
      ...
      $Script:TxtBuildDeployHint.Text=((L 'Blocked by {0} unsupported shared asset(s). Disable one provider above or use HAND TO AI / MODDER. A patch from a different source set cannot be deployed.' 'Bloqueado por {0} asset(s) compartido(s) no soportado(s). Desactiva un provider arriba o usa ENTREGAR A IA / MODDER. No se puede desplegar un parche de otro conjunto de fuentes.') -f $unsupported.Count)
    }
    return
  }
...
function Refresh-UI {
  ...
  $Script:TxtLibraryPath.Text = Join-Path $Script:Root 'Mods'
  ...
  try{
    ...
    if([string]$gr.Status -eq 'Current'){
      $Script:TxtGameReferenceSummary.Text=((L 'Status: Current | {0} families | {1} files | {2} | built {3}. AI_HANDOFF can include related Vanilla context.' 'Estado: Actual | {0} familias | {1} archivos | {2} | creado {3}. AI_HANDOFF puede incluir contexto Vanilla relacionado.') -f [int]$gr.FamilyCount,[int]$gr.FileCount,$grSize,$grCreated)
      ...
      $Script:TxtGameReferenceSummary.Text=L 'Status: Not built. Build it once to give future AI_HANDOFF ZIPs a reusable Vanilla neighborhood from your installed game.' 'Estado: No creada. Creala una vez para dar a futuros AI_HANDOFF un contexto Vanilla reutilizable desde tu juego instalado.'
    }else{
...
function Handle-UIError($ErrorRecord,[string]$Action) {
  ...
  Show-Error (L "$Action failed:`n`n$message`n`nFull details were saved to Logs\PalModMerger.log" "$Action fallo:`n`n$message`n`nEl detalle completo se guardo en Logs\PalModMerger.log")
...
$Script:BtnOpenLibrary.Add_Click({ try { Start-Process explorer.exe -ArgumentList ('"'+(Join-Path $Script:Root 'Mods')+'"') } catch { Handle-UIError $_ (L 'Open library' 'Abrir biblioteca') } })
...
$Script:BtnOpenPatches.Add_Click({try{Start-Process explorer.exe -ArgumentList ('"'+(Join-Path $Script:Root 'Builds')+'"')}catch{Handle-UIError $_ (L 'Open patch folders' 'Abrir carpetas de parches')}})
...
function Open-PMMCurrentAIHandoffAfterAnalyze {
  try{
    Refresh-UI
    Show-SelectedUnsupportedAsset
    $newZip=[string]$Script:BtnOpenAIHandoff.Tag
    if([string]::IsNullOrWhiteSpace($newZip) -or -not(Test-Path -LiteralPath $newZip -PathType Leaf)){
      throw (L 'Analyze completed, but the selected Unsupported case no longer has an AI_HANDOFF. Select the current blocked asset again.' 'El Analisis termino, pero el caso Unsupported seleccionado ya no tiene AI_HANDOFF. Selecciona de nuevo el asset bloqueado actual.')
    }
    Start-Process explorer.exe -ArgumentList ('/select,"'+$newZip+'"')
  }catch{Handle-UIError $_ (L 'AI handoff' 'Entrega para IA')}
}

$Script:BtnOpenAIHandoff.Add_Click({
  try{
    $zip=[string]$Script:BtnOpenAIHandoff.Tag
    if([string]::IsNullOrWhiteSpace($zip) -or -not(Test-Path -LiteralPath $zip -PathType Leaf)){
      throw (L 'No AI_HANDOFF ZIP is available. Run Analyze again.' 'No hay un ZIP AI_HANDOFF disponible. Ejecuta Analizar de nuevo.')
    }

    $gr=Get-PMMGameReferenceState
    if([string]$gr.Status -ne 'Current'){
      $question=((L "Game Reference is {0}. Build/refresh it now and re-run Analyze so this AI_HANDOFF includes related Vanilla context for a completely fresh AI?`n`nThis reads your installed Pal-Windows.pak into PMM's local cache and does not modify Palworld." "Game Reference esta {0}. Crear/actualizarla ahora y volver a Analizar para que este AI_HANDOFF incluya contexto Vanilla relacionado para una IA totalmente nueva?`n`nEsto lee tu Pal-Windows.pak instalado en la cache local de PMM y no modifica Palworld.") -f [string]$gr.Status)
      if(Confirm $question){
        $afterReference={
          param($built)
          try{
            $done={param($result) Open-PMMCurrentAIHandoffAfterAnalyze}
            [void](Start-PMMBackgroundOperation -Operation Analyze -Force -OnSuccess $done)
          }catch{Handle-UIError $_ (L 'AI handoff' 'Entrega para IA')}
        }
        [void](Start-PMMGameReferenceBuild -OnSuccess $afterReference)
        return
      }
    }elseif(-not [string]::IsNullOrWhiteSpace([string]$gr.CreatedUtc)){
      try{
        $handoffUtc=(Get-Item -LiteralPath $zip).LastWriteTimeUtc
        $referenceUtc=([datetime]$gr.CreatedUtc).ToUniversalTime()
        if($handoffUtc -lt $referenceUtc){
          $regenerate=Confirm (L 'This AI_HANDOFF was generated before the current Game Reference. Re-run Analyze now to rebuild the handoff with the richer Vanilla context?' 'Este AI_HANDOFF se genero antes que la Game Reference actual. Volver a Analizar ahora para reconstruir el handoff con el contexto Vanilla ampliado?')
          if($regenerate){
            $done={param($result) Open-PMMCurrentAIHandoffAfterAnalyze}
            [void](Start-PMMBackgroundOperation -Operation Analyze -Force -OnSuccess $done)
            return
...
$Script:BtnBuildGameReference.Add_Click({
  try{
    ...
    $question=if([string]$gr.Status -eq 'Current'){
      L 'Rebuild the local Game Reference now? This reads Pal-Windows.pak in the background and replaces only PMM Data\GameReference. Palworld is never modified.' 'Volver a crear Game Reference local? Esto lee Pal-Windows.pak en segundo plano y solo sustituye PMM Data\GameReference. Palworld no se modifica.'
    }else{
    ...
    $onSuccess={
      param($built)
      Show-Info ((L 'Game Reference ready: {0} families, {1:N1} MiB. Future AI_HANDOFF ZIPs can now include relevant Vanilla context.' 'Game Reference lista: {0} familias, {1:N1} MiB. Los futuros AI_HANDOFF ya pueden incluir contexto Vanilla relevante.') -f [int]$built.FamilyCount,([double]$built.Bytes/1MB))
    }
...
$Script:BtnOpenKnowledge.Add_Click({try{Start-Process explorer.exe -ArgumentList ('"'+(Join-Path $Script:Root 'Knowledge')+'"')}catch{Handle-UIError $_ (L 'Open Knowledge library' 'Abrir biblioteca Knowledge')}})
$Script:BtnOpenReviewCases.Add_Click({try{$p=Join-Path $Script:Root 'Data\Review';if(-not(Test-Path -LiteralPath $p -PathType Container)){New-Item -ItemType Directory -Force -Path $p|Out-Null};Start-Process explorer.exe -ArgumentList ('"'+$p+'"')}catch{Handle-UIError $_ (L 'Open AI review cases' 'Abrir casos para IA')}})
$Script:BtnExportKnowledgeContribution.Add_Click({
  try{
    ...
    $warning=L "Only create a runtime contribution after you tested this exact imported solution in Palworld and the expected behaviors worked. Mark this case as a user-reported runtime PASS and package it for maintainer/community validation?`n`nThe package can contain the original handoff, source mod PAKs and locally extracted Vanilla reference files. Send it to the PMM maintainer/approved private intake, not as a public asset mirror.`n`nThis does NOT auto-authorize a Knowledge recipe on this PC." "Crea una contribucion runtime solo despues de probar esta solucion importada exacta dentro de Palworld y confirmar los comportamientos esperados. Marcar este caso como PASS runtime reportado por el usuario y empaquetarlo para validacion comunitaria/mantenedor?`n`nEl paquete puede contener el handoff original, PAK fuente de mods y archivos Vanilla extraidos localmente. Envialo al mantenedor/servicio privado aprobado de PMM, no como espejo publico de assets.`n`nEsto NO autoriza automaticamente una receta Knowledge en este PC."
    if(-not(Confirm $warning)){return}
...
$Script:BtnSetupDeps.Add_Click({ try { & "$Script:Root\Setup-Dependencies.ps1"; Refresh-UI; Show-Info(L 'Dependency preparation finished.' 'Proceso de dependencias terminado.') } catch { Handle-UIError $_ (L 'Dependency preparation' 'Preparacion de dependencias') } })
...
Refresh-UI
if (-not $autoDepsOk) {
  Show-Info (L 'Some dependencies are still unavailable. Restart with Start.cmd or use Settings > Prepare / repair dependencies.' 'Aun faltan algunas dependencias. Reinicia con Start.cmd o usa Configuracion > Preparar / reparar dependencias.')
}
