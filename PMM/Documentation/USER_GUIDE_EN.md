# Palworld Manager Merger v1.2.1 — User Guide

**Guided workflow:** PMM highlights only the next useful stage in the normal path: **Import -> Analyze -> Build -> Deploy**. The color is state-derived, so a completed/current step returns to the normal neutral button style.
...
9. If **Blocked shared assets** reports Unsupported, either disable an involved mod and Analyze again or use **CREATE AI HANDOFF** for the advanced workflow.
...
In **Settings**, **Build / refresh Game Reference** remains an optional local research
cache. PMM reads selected material from your installed `Pal-Windows.pak` into
`Workspace/GameReference` and never modifies the game.
