...
Unsupported -> `AI_HANDOFF_<case>.zip` -> AI/modder returns `PMM_MANUAL_SOLUTION_V1` -> PMM validates/imports -> user tests in Palworld -> maintainer records runtime result -> only then may an exact recipe or stronger generic adapter be added.
...
- `Core/MergeEngine.ps1` — Analyze/Build routing
...
`Write-PMMAIHandoff` enriches Unsupported packages with a bounded supporting
`references/` neighborhood when Game Reference is Current. The receiver is explicitly
assumed to have zero previous chat context. This is an evidence feature and does not
change the merger's authorization path.
