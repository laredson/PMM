...
9. Si **Blocked shared assets** muestra Unsupported, desactiva uno de los mods implicados y vuelve a Analyze o usa **HAND TO AI / MODDER** para el flujo avanzado.
...
Empieza por `Documentation/TROUBLESHOOTING.md` y `Logs/PalModMerger.log`. Al reportar un problema incluye la versión de PMM, el error exacto y el AI_HANDOFF/review case relevante cuando exista.

## Game Reference Vanilla y handoffs IA con más contexto

En **Configuración**, usa **Crear / actualizar Game Reference** una vez después de
seleccionar Palworld. PMM lee tu `Pal-Windows.pak` instalado, hace una extracción rápida
por lotes dentro de su propia caché `Data/GameReference` y nunca modifica el juego. Si
cambia Palworld, los mappings o el alcance de referencia, PMM la marca como Desactualizada.

Cuando uses **HAND TO AI / MODDER**, PMM incluirá sólo un subconjunto Vanilla relevante
de esa referencia y contexto adicional de providers pequeños. De esta forma una IA nueva
puede estudiar ejemplos relacionados del juego sin conocer tus chats anteriores.
