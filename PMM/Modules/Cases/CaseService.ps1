...
function Add-PMMCaseEvidenceRevision([string]$CaseId,$Evidence) {
  ...
  # Bind the persisted JSON value, not PowerShell's transient wrapper types
  # (e.g. AutomationNull from an empty pipeline becomes {} when serialized).
  # The wrapper preserves empty arrays and null at the document boundary.
  $Evidence=(ConvertFrom-Json -InputObject (ConvertTo-Json -InputObject ([ordered]@{Evidence=$Evidence}) -Depth 95)).Evidence
...
function Sync-PMMCasesFromAnalysis {
  ...
  foreach($asset in @($Plan.Assets|Where-Object{[string]$_.Mode -eq 'Unsupported'})){
    ...
    $evidence=[ordered]@{
      ...
      LayoutEvidence=Get-PMMCaseValue $asset 'LayoutEvidence' $null
    }
  ...
  foreach($entry in $prepared){
    ...
    $providerNames=@($entry.Evidence.Providers|ForEach-Object{[string]$_.Name})
    $extras=@($case.References.Mods|Where-Object{[string]$_.Name -notin $providerNames})
    $case.References.Mods=@(foreach($provider in $entry.Evidence.Providers){
      $prior=@($case.References.Mods|Where-Object{[string]$_.Name -ceq [string]$provider.Name}|Select-Object -First 1)
      $reference=if($prior.Count){Copy-PMMAIIOCaseObject $prior[0]}else{[pscustomobject]@{Kind='MOD';LogicalId=$provider.LogicalId;Name=$provider.Name;Path='';Sha256=$provider.Sha256;Mode='FAMILY';Families=@()}}
      $reference.Path='';$reference.Sha256=$provider.Sha256
      $reference.Families=@(@(Get-PMMCaseValue $reference 'Families' @())+@([string]$entry.Asset.Asset)|Select-Object -Unique)
      $reference
    })+$extras
    $otherVanilla=@($case.References.VanillaFamilies|Where-Object{[string]$_.LogicalPath -ine [string]$entry.Asset.Asset})
    $case.References.VanillaFamilies=@([pscustomobject]@{Kind='VANILLA_FAMILY';LogicalPath=[string]$entry.Asset.Asset;Mode='FAMILY'})+$otherVanilla
    # Keep the user's objective and supporting references when Analyze refreshes
    # machine evidence. Reanalysis must not silently revert the editable case.
    $entry.Evidence['Context']=Get-PMMCaseContextSnapshot $case
    ...
    $latest=Get-PMMAIIOCaseStep ([string]$case.CaseId) ([int]$case.CurrentStep);if($revision -cne $priorRevision -or -not$latest -or [string](Get-PMMCaseValue $latest.State 'EvidenceRevision' '') -cne $revision){
      $case=Get-PMMAIIOCase $case.CaseId
      Add-PMMAIIOCaseStep $case 'ANALYSIS_EVIDENCE' 'Unsupported asset recorded. Open its case to investigate.' 'EDIT_OR_CREATE_HANDOFF' @((Join-Path (Get-PMMAIIOCasePath $case.CaseId) ('revisions/'+$revision+'.json'))) ([ordered]@{EvidenceRevision=$revision;LegacyReviewCaseId=$entry.Review.CaseId})|Out-Null
    }
  ...
  Sync-PMMCaseAnalysisPresence $Plan
...
function Add-PMMCaseContextSources($Case,$Snapshot,[string]$StageRoot) {
  ...
  foreach($reference in $referenceList){
    ...
    if([string](Get-PMMCaseValue $reference 'Mode' '') -eq 'FAMILY' -and $families.Count){
      ...
      # A full-PAK transport still carries the exact family that establishes
      # this analysis revision; verify it before publishing the bundle.
      $captured=@(Get-PMMCaseValue $revision.Evidence 'InputFiles' @()|Where-Object{[string]$_.Role -eq 'Provider' -and [string]$_.Provider -ceq $name})
      if($captured.Count){
        $logical=[string]$revision.Evidence.Asset
        if([IO.Path]::GetExtension($logical) -ieq '.uasset'){Export-PakAssetFamilyExact $path $logical $destination|Out-Null}else{Export-PakFileExact $path $logical $destination|Out-Null}
      }
    }
...
function Sync-PMMCaseAnalysisPresence($Plan) {
  $byKey=@{}
  foreach($asset in @($Plan.Assets)){
    $providers=@(Get-PMMCaseValue $asset 'Providers' @())
    if($providers.Count){$byKey[(Get-PMMConflictCaseKey ([string]$asset.Asset) $providers)]=$asset}
  }
  foreach($case in @(Get-PMMAIIOCases)){
    if([string](Get-PMMCaseValue $case 'Origin' '') -ne 'Jugar' -or -not[string](Get-PMMCaseValue $case 'LogicalKey' '')){continue}
    $matched=$byKey[[string]$case.LogicalKey]
    $status=if(-not$matched){'NotInCurrentAnalysis'}elseif([string]$matched.Mode -eq 'Unsupported'){'Blocked'}else{'NoLongerBlocked'}
    $previous=[string](Get-PMMCaseValue (Get-PMMCaseValue $case 'AnalysisPresence' $null) 'Status' '')
    $presence=[ordered]@{Status=$status;Mode=$(if($matched){[string]$matched.Mode}else{''});MappingsSha256=[string]$Plan.MappingsSha256;SourceSignature=[string](Get-PMMCaseValue $Plan 'SourceSignature' '');ObservedUtc=[DateTime]::UtcNow.ToString('o');RuntimeProven=$false}
    $case|Add-Member -NotePropertyName AnalysisPresence -NotePropertyValue ([pscustomobject]$presence) -Force
    Save-PMMAIIOCase $case|Out-Null
    if($previous -and $status -ne $previous){
      $summary=switch($status){'Blocked'{'The current analysis blocks this asset again.'};'NoLongerBlocked'{'The current analysis can merge this asset. Game behavior is not yet validated.'};default{'These providers are no longer together in the current analysis; history is preserved.'}}
      Add-PMMAIIOCaseStep $case 'ANALYSIS_STATUS_CHANGED' $summary ([string]$case.NextAction) @() ([ordered]@{AnalysisPresence=$presence})|Out-Null
    }
  }
}
