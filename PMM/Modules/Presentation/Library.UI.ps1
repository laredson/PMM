...
function Show-SelectedUnsupportedAsset {
  ...
  $Script:BtnOpenAIHandoff.Tag=[string]$item.Asset
...
function Invoke-PMMPriorityEditorCommit([System.Windows.Controls.TextBox]$Editor) {
  if(-not [string]::IsNullOrWhiteSpace((Get-PMMActiveProcessingOperation))){return $false}
...
function Apply-PMMLibraryFilter {
  ...
  $selection=@($Script:LstMods.SelectedItems | ForEach-Object {[string]$_.Name})
  $scroll=Get-PMMUiDescendant $Script:LstMods ([Windows.Controls.ScrollViewer]);$offset=if($scroll){$scroll.VerticalOffset}else{0}
  $Script:LstMods.ItemsSource=$items
  foreach($entry in $items){if([string]$entry.Name -in $selection){[void]$Script:LstMods.SelectedItems.Add($entry)}}
  if($scroll){$scroll.ScrollToVerticalOffset($offset)}
...
function Update-PMMLibraryButtons {
  if(-not [string]::IsNullOrWhiteSpace((Get-PMMActiveProcessingOperation))){
...
function Refresh-UI {
  ...
  foreach($mod in $sourceMods){
    $displayItems.Add([pscustomobject]@{Kind='Source';Mod=$mod;CaseId='';CandidateId='';Name=[string]$mod.Name;Enabled=$true;State=(L 'active' 'activo');Priority=[int]$mod.Priority;SizeText=("{0:N2}" -f ($mod.Size/1MB));HashShort=$mod.Hash.Substring(0,12)})
  }
  foreach($mod in $disabledMods){
    $displayItems.Add([pscustomobject]@{Kind='Source';Mod=$mod;CaseId='';CandidateId='';Name=[string]$mod.Name;Enabled=$false;State=(L 'disabled' 'desactivado');Priority=[int]$mod.Priority;SizeText=("{0:N2}" -f ($mod.Size/1MB));HashShort=$mod.Hash.Substring(0,12)})
  }
  if(Get-Command Get-PMMKnowledgeCandidates -ErrorAction SilentlyContinue){
    $generated=@{};foreach($candidate in @(Get-PMMKnowledgeCandidates)){if($candidate.PakSha256){$generated[[string]$candidate.PakSha256]=$candidate}}
    foreach($entry in $displayItems){if($generated.ContainsKey([string]$entry.Mod.Hash)){$candidate=$generated[[string]$entry.Mod.Hash];$entry.CaseId=$candidate.CaseId;$entry.CandidateId=$candidate.CandidateId;$label=switch([string]$candidate.UserConfirmation){'Worked'{L 'Generated - user confirmed' 'Generado - confirmado por usuario'}'Failed'{L 'Generated - reported failure' 'Generado - fallo comunicado'}default{L 'Generated - Pending test' 'Generado - Pendiente de prueba'}};$entry.State=$label+' / '+$entry.State}}
  }
  ...
  try{
    ...
    if([string]$gr.Status -eq 'Current'){
      $Script:TxtGameReferenceSummary.Text=((L 'Status: Current | {0} families | {1} files | {2} | built {3}. The reference files are current. Semantic reading remains unvalidated until the adapter checks the exact requested asset.' 'Estado: Actual | {0} familias | {1} archivos | {2} | creado {3}. Los archivos de referencia estan actualizados. La lectura semantica sigue sin validar hasta que el adaptador compruebe el asset solicitado.') -f [int]$gr.FamilyCount,[int]$gr.FileCount,$grSize,$grCreated)
    }elseif([string]$gr.Status -eq 'Stale'){
      $Script:TxtGameReferenceSummary.Text=((L 'Status: Stale. {0} Refresh it before preparing new case evidence.' 'Estado: Desactualizado. {0} Actualiza la referencia antes de preparar nuevas evidencias del caso.') -f [string]$gr.Reason)
    }elseif([string]$gr.Status -eq 'NotBuilt'){
      $Script:TxtGameReferenceSummary.Text=L 'Status: Not built. Optional local research/reference cache; Individual assets still require adapter validation.' 'Estado: No creada. Cache local opcional de investigacion/referencia; Cada asset necesita la validacion de su adaptador.'
    }else{
