...
function Stop-PMMBackgroundOperation([switch]$Silent) {
  ...
  if($kind -in @('Analyze','DeepAnalysis')){Set-PMMAnalyzeBusy $false}
...
function Complete-PMMBackgroundOperation {
  ...
  $kind=[string]$Script:BackgroundOperationKind
  $completedLease=$null
  if(Get-Variable PMMUIOperationLease -Scope Script -ErrorAction SilentlyContinue){$completedLease=$Script:PMMUIOperationLease;$Script:PMMUIOperationLease=$null}
  try{
  $result=$null
  try{
  ...
  if($result -and [bool]$result.Success){
    ...
    if($kind -in @('Analyze','DeepAnalysis')){Set-PMMAnalyzeProgress 1 1 (L 'Analyze complete.' 'Analisis terminado.')}
    elseif($kind -eq 'Build'){Set-PMMBuildProgress 1 1 (L 'Build complete.' 'Build terminado.')}
    elseif($kind -in @('AIHandoff','AIIOPrepare','AIIOPendingData','AIIOImportResponse','AIIOUseCandidate','AIIOModBuild','AIIOArtifactRefresh','MappingsImport','DeepCase','DeepSource')){Set-PMMAIIOProgress 1 1 (L 'AIIO operation complete.' 'Operacion AIIO terminada.')}
    ...
    $failureOperation=if($kind -in @('AIHandoff','AIIOPrepare','AIIOPendingData','AIIOImportResponse','AIIOUseCandidate','AIIOModBuild','AIIOArtifactRefresh','MappingsImport','DeepCase','DeepSource')){'AIIO'}elseif($kind -eq 'FixLabBuild'){'FixLab'}else{$kind}
  ...
  }finally{if($completedLease){Complete-PMMModuleOperation $completedLease.Id}}
}
...
function Start-PMMBackgroundOperation {
  ...
  if($RequestPath){$args+=' -RequestPath '+(ConvertTo-NativeQuotedArgument $RequestPath)}
  if($Force){$args+=' -Force'}
  if($AllowOversize){$args+=' -AllowOversize'}

  if($Operation -in @('Analyze','DeepAnalysis')){
  ...
  }elseif($Operation -in @('AIHandoff','AIIOPrepare','AIIOPendingData','AIIOImportResponse','AIIOUseCandidate','AIIOModBuild','AIIOArtifactRefresh','MappingsImport','DeepCase','DeepSource')){
  ...
  try{
    $Script:PMMUIOperationLease=Start-PMMModuleOperation $Operation
    ...
    try{
      if(Test-Path -LiteralPath $Script:BackgroundOperationProgressPath -PathType Leaf){
        try{
          ...
          }elseif([string]$progress.Operation -in @('AIHandoff','AIIOPrepare','AIIOPendingData','AIIOImportResponse','AIIOUseCandidate','AIIOModBuild','AIIOArtifactRefresh','MappingsImport','DeepCase','DeepSource')){
