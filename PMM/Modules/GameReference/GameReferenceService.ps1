...
$Script:PMMGameReferenceMappingHashCache = ''
$Script:PMMGameReferenceMappingHashStamp = ''

function Get-PMMGameReferenceRoot { return (Get-PMMPath 'GameReference') }
...
function Get-PMMGameReferenceQuickIdentity {
  ...
  if(Test-Path -LiteralPath $mapping -PathType Leaf){
    # Mappings can be tens of MiB. Hash them only when file identity changes;
    # UI state/guidance queries must be O(1), not re-hash the file on every tab.
    $mi=Get-Item -LiteralPath $mapping -ErrorAction SilentlyContinue
    if($mi){
      $stamp=([IO.Path]::GetFullPath($mi.FullName).ToLowerInvariant()+'|'+[string]$mi.Length+'|'+$mi.LastWriteTimeUtc.Ticks)
      if($stamp -ne [string]$Script:PMMGameReferenceMappingHashStamp){
        $Script:PMMGameReferenceMappingHashCache=Get-Sha256 $mi.FullName
        $Script:PMMGameReferenceMappingHashStamp=$stamp
      }
      $mappingHash=[string]$Script:PMMGameReferenceMappingHashCache
    }
  }
  if(-not$pak){
...
function Build-PMMGameReferenceLibrary {
  ...
  $referenceLockPath=Join-Path $root 'GameReference.build.lock'
  $referenceLock=$null
  try{$referenceLock=[IO.File]::Open($referenceLockPath,[IO.FileMode]::OpenOrCreate,[IO.FileAccess]::ReadWrite,[IO.FileShare]::None)}catch{
    throw 'Game Reference is already being built by another PMM process.'
  }
...
function Get-PMMGameReferenceRelationRules {
  $path=Get-PMMCKLStablePath 'reference-relations.json'
  if(-not(Test-Path -LiteralPath $path -PathType Leaf)){return @()}
...
function Get-PMMGameReferenceProviderSeedInfo([array]$ProviderRecords,[string]$ConflictAsset) {
  ...
  foreach($record in @($ProviderRecords)){
    ...
    try{
      ...
      # paths seed. AIIO never embeds the complete source PAK; only indexes/exact
      # selected families may be used as supporting evidence by this legacy helper.
...
function Add-PMMProviderContextToHandoff([string]$StageRoot,[array]$ProviderRecords,[string]$ConflictAsset) {
  ...
  foreach($record in @($ProviderRecords)){
    ...
    try{
      ...
      # Broader providers keep only an entry index here; whole source PAKs are
      # never embedded in a handoff.
...
# =============================================================================
# PMM 1.3 Game Reference extensions for Fix Lab / future AIIO
# =============================================================================

function Get-PMMGameReferenceVersionsRoot {
  $p=Join-Path (Get-PMMGameReferenceRoot) 'versions'
  if(-not(Test-Path -LiteralPath $p -PathType Container)){New-Item -ItemType Directory -Force -Path $p|Out-Null}
  return $p
}

function Get-PMMGameReferenceVersionKey($State) {
  if(-not$State){return ('reference_'+(Get-Date -Format 'yyyyMMdd_HHmmss'))}
  $pak=([string]$State.PakIndexSha256).ToLowerInvariant();if($pak.Length -gt 12){$pak=$pak.Substring(0,12)}
  $map=([string]$State.MappingsSha256).ToLowerInvariant();if($map.Length -gt 12){$map=$map.Substring(0,12)}
  if([string]::IsNullOrWhiteSpace($pak)){$pak='unknownpak'};if([string]::IsNullOrWhiteSpace($map)){$map='unknownmap'}
  return ('pak_'+$pak+'_map_'+$map)
}

function Archive-PMMGameReferenceVersion([string]$Folder,$NewState=$null) {
  if(-not(Test-Path -LiteralPath $Folder -PathType Container)){return ''}
  $statePath=Join-Path $Folder 'state.json';$old=$null
...
function Get-PMMGameReferenceVersions {
...
function Ensure-PMMGameReferenceFamilies {
  param([Parameter(Mandatory=$true)][string[]]$Assets,[string]$Reason='On-demand request')
  $state=Get-PMMGameReferenceState
  if([string]$state.Status -ne 'Current'){throw (Get-PMMText 'Current Game Reference is required before requesting additional families.' 'Se requiere la Game Reference actual antes de solicitar familias adicionales.')}
  $pak=Get-PMMGameReferencePak;if([string]::IsNullOrWhiteSpace($pak)){throw 'Pal-Windows.pak is unavailable.'}
  $cooked=Get-PMMGameReferenceCookedRoot;$added=[System.Collections.Generic.List[string]]::new();$requested=[System.Collections.Generic.List[string]]::new()
  foreach($asset0 in @($Assets|Where-Object{$_}|Sort-Object -Unique)){
    $asset=Normalize-PMMReferenceLogicalPath ([string]$asset0)
    if([IO.Path]::GetExtension($asset) -ine '.uasset'){throw ('Game Reference on-demand family must name a .uasset: '+$asset)}
    $requested.Add($asset)
    $stem=Get-PakLogicalStem $asset;$needs=$false
    $entries=@(Get-PakEntriesCached $pak)
    foreach($ext in @('.uasset','.uexp','.ubulk')){
      $entry=Find-PakEntryExact $entries ($stem+$ext);if(-not$entry){continue}
      $local=Join-Path $cooked (($stem+$ext).Replace([char]47,[char]92));if(-not(Test-Path -LiteralPath $local -PathType Leaf)){$needs=$true;break}
    }
    if($needs){[void](Export-PakAssetFamilyExact $pak $asset $cooked);$added.Add($asset)}
  }
  if($added.Count -gt 0){
    $stats=Write-PMMGameReferenceFamilyIndex $cooked (Get-PMMGameReferenceIndexRoot)
    $statePath=Join-Path (Get-PMMGameReferenceCurrentRoot) 'state.json';$s=Get-Content -LiteralPath $statePath -Raw|ConvertFrom-Json
    $s.ExtractedFamilyCount=[int]$stats.FamilyCount;$s.ExtractedFileCount=[int]$stats.FileCount;$s.ExtractedBytes=[int64]$stats.Bytes
    if(-not($s.PSObject.Properties.Name -contains 'OnDemandUpdatedUtc')){$s|Add-Member -NotePropertyName OnDemandUpdatedUtc -NotePropertyValue ''}
    if(-not($s.PSObject.Properties.Name -contains 'OnDemandFamilyRequests')){$s|Add-Member -NotePropertyName OnDemandFamilyRequests -NotePropertyValue 0}
    $s.OnDemandUpdatedUtc=[DateTime]::UtcNow.ToString('o');$s.OnDemandFamilyRequests=[int]$s.OnDemandFamilyRequests+$added.Count
    $s|ConvertTo-Json -Depth 20|Set-Content -LiteralPath $statePath -Encoding UTF8;Copy-Item -LiteralPath $statePath -Destination (Get-PMMGameReferenceStatePath) -Force
    $Script:PMMGameReferenceFamilyCache=$null;$Script:PMMGameReferenceFamilyCacheStamp=''
  }
  $logPath=Join-Path (Get-PMMGameReferenceIndexRoot) 'on-demand-requests.jsonl'
  $row=[ordered]@{Utc=[DateTime]::UtcNow.ToString('o');Reason=$Reason;Requested=$requested.ToArray();Extracted=$added.ToArray()}
  Add-Content -LiteralPath $logPath -Value ($row|ConvertTo-Json -Depth 8 -Compress) -Encoding UTF8
  return [pscustomobject]@{Requested=$requested.Count;Extracted=$added.Count;Assets=$requested.ToArray();Added=$added.ToArray()}
}
