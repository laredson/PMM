...
$fallbackCaseDir=Join-Path $Script:Root ('Workspace\AIIO\Cases\'+$CaseId)

function Write-PMMAIIOWorkerBootstrapFailure([string]$Message,[string]$Details=''){
  try{
    New-Item -ItemType Directory -Force -Path $fallbackCaseDir|Out-Null
    $text=$Message
    if($Details){$text+=[Environment]::NewLine+[Environment]::NewLine+$Details}
    $text|Set-Content -LiteralPath (Join-Path $fallbackCaseDir 'worker-startup-error.txt') -Encoding UTF8
    [ordered]@{Schema='PMM_AIIO_CASE_WORKER_RESULT_V1';CaseId=$CaseId;Mode=$Mode;Status='Failed';Phase='Bootstrap';Message=$Message;ZipPath='';Utc=[DateTime]::UtcNow.ToString('o')}|ConvertTo-Json -Depth 12|Set-Content -LiteralPath (Join-Path $fallbackCaseDir 'worker-result.json') -Encoding UTF8
  }catch{}
}

# Module loading is part of the supervised worker lifecycle. Previously a
# StrictMode error here could kill the process before worker-result.json existed.
try{
  ...
  . (Join-Path $Script:Root 'Modules\Cases\CaseService.ps1')
}catch{
...
function Write-PMMAIIOWorkerResult([string]$Status,[string]$Message,[string]$ZipPath=''){
  try{
    ...
    Write-PMMAIIOCaseJson $path ([ordered]@{Schema='PMM_AIIO_CASE_WORKER_RESULT_V1';CaseId=$CaseId;Mode=$Mode;Status=$Status;Phase='Run';Message=$Message;ZipPath=$ZipPath;Utc=[DateTime]::UtcNow.ToString('o')}) 12
  }catch{
    try{Write-PMMAIIOWorkerBootstrapFailure $Message ($_|Out-String)}catch{}
  }
}

try{
  Remove-Item -LiteralPath (Join-Path (Get-PMMAIIOCasePath $CaseId) 'worker-startup-error.txt') -Force -ErrorAction SilentlyContinue
  ...
  if($Mode-eq'HANDOFF'){
    ...
    $guard=0
    while($guard-lt30){
      $guard++;$case=Get-PMMAIIOCase $CaseId
      ...
      foreach($action in @(Get-PMMAIIOActionArray $case 'PendingActions')){if([string](Get-PMMAIIOActionValue $action 'Status' 'Pending')-eq'Pending'){$pending.Add($action)}}
      if($pending.Count-gt0 -and $next-ne'PROCESS_REQUESTS'){$case.NextAction='PROCESS_REQUESTS';Save-PMMAIIOCase $case|Out-Null;$next='PROCESS_REQUESTS'}
      if($next-eq'PROCESS_REQUESTS'){[void](Invoke-PMMAIIOCasePendingActions $CaseId);continue}
      if($next-in@('CREATE_HANDOFF','EDIT_OR_CREATE_HANDOFF')){
        $case=Get-PMMAIIOCase $CaseId;$step=[int](Get-PMMAIIOActionValue $case 'CurrentStep' 0)
