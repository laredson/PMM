...
function Invoke-PMMAIIOFetchLink([string]$Id,$Raw,[string]$Evidence){
  ...
  $request.Method='GET';$request.AllowAutoRedirect=$true;$request.UserAgent='Palworld-Manager-Merger-AIIO/1.3.1'
  ...
  try{
    ...
    try{if($input){$input.Dispose()}}catch{}
  ...
  $info=Get-Item -LiteralPath $target;$sha=(Get-Sha256 $target).ToLowerInvariant()
...
function New-PMMAIIOCaseHandoff([string]$Id,[int]$FromStep=0){
  ...
  $zip=[string]$result.ZipPath;if(-not(Test-Path -LiteralPath $zip -PathType Leaf)){return$result}
