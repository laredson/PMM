...
function New-PMMAIIOCaseHandoff([string]$Id,[int]$FromStep=0){
  $result=& $Script:PMMAIIORemoteBaseHandoff $Id $FromStep;$zip=[string]$result.ZipPath;if([string]::IsNullOrWhiteSpace($zip) -or -not(Test-Path -LiteralPath $zip -PathType Leaf)){return $result}
