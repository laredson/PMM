...
function Invoke-PMMAIIOAutoCaseUiV4 {
  ...
  if($case.Transport -eq 'MCP' -and (Get-PMMCaseClient $case) -in @('CHATGPT','CODEX_DESKTOP')){Show-PMMChatGPTCase;return}
  if($case.Transport -eq 'MCP' -and (Get-PMMCaseClient $case) -eq 'CODEX'){(Get-PMMAIIOCaseControl 'PnlMCPReply').Visibility='Visible';(Get-PMMAIIOCaseControl 'ChatTabs').SelectedIndex=1;(Get-PMMAIIOCaseControl 'ChatAdvanced').IsExpanded=$true;Update-PMMChatPanel $case;return}
