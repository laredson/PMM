...
function Import-PMMMod([string]$Path,[scriptblock]$ProgressCallback=$null) {
  ...
  $sourceArchiveSha256=Get-Sha256 $Path
  $sourceArchiveMd5=(Get-FileHash -LiteralPath $Path -Algorithm MD5).Hash.ToLowerInvariant()
  ...
  try {
    ...
    if((Get-Sha256 $Path) -cne $sourceArchiveSha256){throw 'Import source changed during extraction.'}
    ...
    foreach ($pak in $paks) {
      ...
      [pscustomobject]@{
        ...
        ContentSha256=$hash
        ContentPath=$pak.FullName.Substring($stage.Length).TrimStart('\')
