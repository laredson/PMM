...
func startupState(state string) (startupView, bool) {
	...
	case s == "startup:runtime-dependency-failed" || strings.HasPrefix(s, "startup:UI-runtime-exit:"):
...
func nextStartupView(previous startupView, state string) startupView {
	...
	if !known || previous.Ready || previous.Failed {
		return previous
	}
	if next.Failed || next.Stage >= previous.Stage {
		return next
	}
	return previous
}
