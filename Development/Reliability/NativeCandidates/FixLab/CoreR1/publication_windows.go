...
func candidateParent(parent *os.File) (*os.File, error) {
	f, e := candidateNt(parent, ".", true, false, true, 0)
	if e != nil {
		return nil, e
	}
...
func candidateCommit(parent, stage *os.File, old, new string) (bool, error) {
	...
	ok, _, e := candidateSetInfo.Call(stage.Fd(), 3, uintptr(unsafe.Pointer(&raw[0])), uintptr(len(raw)))
	runtime.KeepAlive(parent)
