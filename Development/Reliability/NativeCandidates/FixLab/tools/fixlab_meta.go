...
func inspect(path string) (_ map[string]any, err error) {
	...
	for _, fn := range table.Funcs {
		...
		source, line, _ := table.PCToLine(fn.Entry)
		funcs = append(funcs, function{fn.Name, fn.Entry, fn.End, fn.End - fn.Entry, digest(text[fn.Entry-base : fn.End-base]), source, line})
	}
	if len(funcs) == 0 {
	...
	return map[string]any{
		"schema": "PMM_FIXLAB_GO_STATIC_V1", "sha256": digest(b), "goVersion": bi.GoVersion,
...
func main() {
	if len(os.Args) != 2 {
		fmt.Fprintln(os.Stderr, "usage: fixlab_meta <Go-Windows-EXE>")
		os.Exit(2)
	}
