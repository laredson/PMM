...
func executableRoot() (string, error) {
	...
	if strings.EqualFold(filepath.Base(dir), "Engine") {
...
func logLine(root, area, msg string) {
	...
	fmt.Println(line)
	logDir := filepath.Join(root, "Workspace", "Logs")
...
func cleanRelPath(rel string) (string, error) {
	// PMM targets Windows even when CI runs on Linux. Normalize both separator
	// forms first, then enforce Windows filename semantics explicitly so a path
	// that is harmless on Linux cannot turn into traversal, an ADS, or a device
	// name when the exact same archive is opened by PMM on Windows.
	...
	for _, segment := range strings.Split(trimmed, "/") {
		...
		if len(segment) > 255 {
			return "", fmt.Errorf("unsafe relative path component is too long: %q", rel)
		}
		...
		if base == "CON" || base == "PRN" || base == "AUX" || base == "NUL" ||
			...
			return "", fmt.Errorf("unsafe Windows device path: %q", rel)
		}
	}
