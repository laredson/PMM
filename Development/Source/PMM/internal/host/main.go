...
import (
	...
	"pmm.local/pmm/internal/supervision"
	"syscall"
	"time"
	"unsafe"
)
...
func (h *Host) run(operation string, args []string) int {
	...
	if operation == "start" && plan.Kind == "native" && len(cmd.Args) >= 3 && strings.EqualFold(cmd.Args[1], "runtime") && strings.EqualFold(cmd.Args[2], "start") {
	...
	if b, err := os.ReadFile(routesPath); err == nil {
		...
		if json.Unmarshal(b, &manifest) == nil && manifest.Schema == "PMM_HOST_ROUTES_V1" {
			if route, ok := manifest.Routes[operation]; ok {
				...
				if kind == "native" {
					if strings.EqualFold(filepath.ToSlash(filepath.Clean(route.Executable)), "Engine/PMMRuntime.exe") {
						self, err := os.Executable()
						if err != nil {
							return ChildPlan{}, fmt.Errorf("cannot resolve unified PMM executable: %w", err)
						}
						argv := append([]string{"runtime"}, route.Arguments...)
						argv = append(argv, args...)
						return ChildPlan{Kind: "native", Name: "PMM.exe runtime", Cmd: exec.Command(self, argv...)}, nil
					}
