...
## AIIO handoff and manual solution contract

Analyze and AI handoff packaging are separate operations.

Analyze writes only durable analysis metadata/reports under `Data/Review/<asset-id>/`.
Cooked Vanilla/provider payload is kept only in the short-lived Analyze transaction and
is deleted when Analyze ends. Analyze never creates an AI handoff ZIP and never copies
whole source PAKs.

When the user explicitly chooses **CREATE AI HANDOFF** (or accepts the post-Analyze
prompt), `Core/AIIO.ps1` creates one `AI_HANDOFF_<bundleId>.zip` for the complete current
Unsupported set. AIIO re-extracts only each exact conflicting file/family from every
involved source PAK and from installed Vanilla, preserving the logical game path under
separate `sources/<origin>/` roots. Whole source PAKs are never included.

AIIO uses a 512 MiB normal ZIP target and a 5 GiB normal uncompressed-bundle limit.
Oversize creation requires explicit user approval. The native `PMMRuntime.exe archive
create` path writes the ZIP; stage and partial files are always cleaned in `finally`, and
startup hygiene removes abandoned Analyze/AIIO staging and legacy handoff payloads.

The bundle contains per-case identities and hash manifests. The current returned cooked
solution mode remains `replacement-cooked-family` under schema
`PMM_MANUAL_SOLUTION_V1`; returned solutions are still imported/validated per exact
`caseId` even though all Unsupported cases arrive in one handoff.
...
4. current mappings and exact provider PAK name+SHA-256 still match;
5. PMM re-extracts the current Vanilla/provider cooked family and rechecks recorded
   topology, size and SHA-256 instead of trusting stored review payload;
6. exact returned asset-family topology and no unrelated cooked files;
7. read-only AssetReader probe;
8. output SHA-256 pinning in `Data/ManualSolutions/<caseId>`.

A validated solution is still marked `UNPROVEN` at runtime and requires explicit user
risk acceptance. Build rechecks stored hashes and AssetReader probe.
