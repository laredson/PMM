...
def validate_identity() -> None:
    ...
    assert manifest["releaseDate"] == "2026-09-11"
...
def validate_dialog_and_feedback_flow() -> None:
    ...
    assert "$Script:AIHelpTabs.SelectedItem=$Script:PMMHelpFeedbackTab" in open_feedback
...
def validate_ai_assistance_and_reception() -> None:
    ...
    assert all("$tab -eq '" + tag + "'" in refresh for tag in ("CASES", "FEEDBACK", "THEME"))
