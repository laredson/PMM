...
foreach($dir in @('Modules\MCP','Modules\Unreal','Resources\Unreal')){
    foreach($file in Get-ChildItem (Join-Path $root $dir) -Recurse -File){
        if($file.Extension -notin @('.ps1','.py','.json')){throw 'Unexpected file in optional module.'}
        [void]$names.Add($file.FullName.Substring($root.Length+1).Replace('\','/'))
    }
}
foreach($name in @('Modules/Shared/Dialogs.ps1','Modules/AIIO/AIIO.Workspaces.UI.ps1')){[void]$names.Add($name)}
...
foreach($doc in @('MCP_BRIDGE.md','UNREAL_SETUP.md','UNREAL_SETUP.es.md','UNREAL_SETUP.en.md')){[void]$names.Add('Documentation/'+$doc)}
...
[IO.File]::WriteAllText($path,(($lines -join [char]10)+[char]10),[Text.UTF8Encoding]::new($false))
