...
try{
  ...
  $origin=Get-OrCreate-PMMOriginCase -Origin Theme -SourceId 'draft-one' -Title 'Theme draft' -Context ([ordered]@{Color='blue'})
  $originRepeat=Get-OrCreate-PMMOriginCase -Origin Theme -SourceId 'draft-one' -Title 'Theme draft' -Context ([ordered]@{Color='blue'})
  Assert-CaseTest ($origin.CaseId -eq $originRepeat.CaseId -and $origin.CurrentEvidenceRevision -eq $originRepeat.CurrentEvidenceRevision) 'module origin is idempotent'
  $origin.Description='Keep this user goal';Save-PMMAIIOCase $origin|Out-Null;Update-PMMCaseContextRevision $origin.CaseId|Out-Null
  $originRefresh=Get-OrCreate-PMMOriginCase -Origin Theme -SourceId 'draft-one' -Title 'Updated theme title' -Description 'Replace me' -Context ([ordered]@{Color='red'})
  Assert-CaseTest ($originRefresh.Description -eq 'Keep this user goal') 'module refresh preserves user goal'
  Assert-CaseTest ((Get-PMMCaseEvidenceRevision $origin.CaseId).Evidence.CaseContext.Objective -eq 'Keep this user goal') 'module evidence binds latest user goal'
  $plan.Assets=@($first);$plan.MappingsSha256=Get-Sha256 (Get-PMMMappingsPath)
  Sync-PMMCasesFromAnalysis $plan -Completed|Out-Null
  $editable=Get-PMMCaseForAsset $first;$editable.Description='Investigate only breeding';Save-PMMAIIOCase $editable|Out-Null
  $editable.References.Mods+=,[pscustomobject]@{Kind='MOD';Name=$mods[2].Name;Path=$mods[2].Path;Sha256=$mods[2].Hash;Mode='FULL_PAK';Families=@()};Save-PMMAIIOCase $editable|Out-Null
  Sync-PMMCasesFromAnalysis $plan -Completed|Out-Null
  $refreshed=Get-PMMAIIOCase $editable.CaseId
  Assert-CaseTest ($refreshed.Description -eq 'Investigate only breeding' -and @($refreshed.References.Mods).Count -eq 3) 'Analyze preserves objective and extra supporting mod'
  $refreshed.References.Mods[0].Mode='FULL_PAK';$refreshed.References.Mods[1].Families+=@('Pal/Content/support.txt');Save-PMMAIIOCase $refreshed|Out-Null
  Sync-PMMCasesFromAnalysis $plan -Completed|Out-Null;$refreshed=Get-PMMAIIOCase $editable.CaseId
  Assert-CaseTest ($refreshed.References.Mods[0].Mode -eq 'FULL_PAK' -and $refreshed.References.Mods[1].Families -contains 'Pal/Content/support.txt') 'Analyze preserves source mode and supporting families'
  $stage=Join-Path $fixture 'full-pak-case';[void][IO.Directory]::CreateDirectory($stage)
  Add-PMMCaseContextSources $refreshed ([pscustomobject]@{References=$refreshed.References;EvidenceRevision=$refreshed.CurrentEvidenceRevision}) $stage
  Assert-CaseTest ((Test-Path (Join-Path $stage 'sources/provider-1/Alpha.pak')) -and (Test-Path (Join-Path $stage 'sources/provider-1/Pal/Content/first.txt'))) 'full-PAK transport preserves captured family evidence'
  $first.Mode='DataTableAuto';Sync-PMMCasesFromAnalysis $plan -Completed|Out-Null
  Assert-CaseTest ((Get-PMMAIIOCase $editable.CaseId).AnalysisPresence.Status -eq 'NoLongerBlocked') 'later successful merge is visible in case history'
  $plan.Assets=@();Sync-PMMCasesFromAnalysis $plan -Completed|Out-Null
  Assert-CaseTest ((Get-PMMAIIOCase $editable.CaseId).AnalysisPresence.Status -eq 'NotInCurrentAnalysis') 'removed source set is distinguished from a successful merge'
