...
2. Run `Start.cmd` (first launch may prepare/download the pinned dependencies).
...
If a new conflict is Unsupported, advanced users can create an **AI_HANDOFF** package containing the relevant files and analysis for that exact case. A developer or capable AI can investigate it and return a solution in PMM's documented format.
