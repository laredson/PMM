<# Palworld Manager Merger v1.2 RC1 source-only smoke test. It never downloads/builds dependencies. #>
param([switch]$Quiet)
...
$validationLogPath=Join-Path $Root 'Logs\Validation.log'
function Write-ValidationLog([string]$Level,[string]$Message){
  try{
    $dir=Split-Path -Parent $validationLogPath
    if(-not(Test-Path -LiteralPath $dir -PathType Container)){New-Item -ItemType Directory -Force -Path $dir|Out-Null}
    $stamp=(Get-Date).ToString('yyyy-MM-dd HH:mm:ss.fff')
    Add-Content -LiteralPath $validationLogPath -Value ("[{0}] [{1}] {2}" -f $stamp,$Level,$Message) -Encoding UTF8
  }catch{}
}
function Fail([string]$m){$failures.Add($m);Write-ValidationLog 'FAIL' $m}
function Note([string]$m){Write-ValidationLog 'INFO' $m;if(-not$Quiet){Write-Host "[Smoke] $m"}}
Write-ValidationLog 'START' 'Palworld Manager Merger v1.2 RC1 release validation started.'

$required=@(
  'Start-PalModMerger.ps1','Core\Common.ps1','Core\GameLocator.ps1','Core\PakService.ps1','Core\LibraryService.ps1','Core\SaveService.ps1','Core\MergeEngine.ps1','Core\SemanticLab.ps1','Core\GameReferenceService.ps1','Core\GameReferenceWorker.ps1','Core\OperationWorker.ps1','Core\KnowledgeRecipeService.ps1','Core\KnowledgeContributionService.ps1','BUILD_ID.txt','PMM.exe','Host\main.go','Host\go.mod','PMMRuntime.exe','Runtime\main.go','Runtime\go.mod','Runtime\README.md','Runtime\runtime-contract.json','Runtime\BUILD_RUNTIME.cmd','Runner\PMM-Runner.ps1','Runner\Operations\start.ps1','Runner\Operations\probe.ps1','Runner\Operations\validate.ps1','START_LEGACY.cmd',
  'Setup-Dependencies.ps1','SETUP_ONCE.cmd','RUN_VALIDATION.cmd','Tools\PMMCore\src\PMM.Core\PMM.Core.csproj','Tools\PMMCore\src\PMM.Cli\PMM.Cli.csproj',
  ...
  'Tools\AssetReader\Program.cs','Tools\AssetReader\AssetReader.csproj','UI\MainWindow.xaml','UI\MainWindow.en.xaml','UI\MainWindow.es.xaml','RELEASE_MANIFEST.json','Knowledge\README.md','Knowledge\package-rules.json','Knowledge\behavior-symbols.json','Knowledge\known-fixtures.json','Knowledge\known-behaviors.json','Knowledge\production-recipes.json','Knowledge\reference-relations.json','Docs\COMMUNITY_KNOWLEDGE_WORKFLOW.md','Docs\COMMUNITY_KNOWLEDGE_SERVER_SPEC.md','Docs\MANUAL_SOLUTION_CONTRACT.md','Documentation\GAME_REFERENCE_AND_COMMUNITY_CONTRIBUTIONS.md','Documentation\README.md','Documentation\AI_DEVELOPMENT_HANDOFF.md','Documentation\V1_1_1_RELEASE_VALIDATION.md','Documentation\V1_2_RC1_VALIDATION.md','Documentation\V1_2_RUNTIME_MIGRATION.md','Runner\routes.json','UI\native-shell.json','Runtime\native_shell_windows.go','Runtime\native_shell_other.go','Runtime\native_shell_model.go','Documentation\ThirdParty\Go-LICENSE.txt','Documentation\USER_GUIDE_EN.md','Documentation\USER_GUIDE_ES.md','TEST_THIS_BUILD.txt','LICENSE','THIRD_PARTY_NOTICES.md'
)
...
if($versionText -ne 'v1.2-rc1'){Fail "VERSION.txt mismatch: $versionText"}
...
$pakService=Get-Content -LiteralPath (Join-Path $Root 'Core\PakService.ps1') -Raw
$xamlDefault=Get-Content -LiteralPath (Join-Path $Root 'UI\MainWindow.xaml') -Raw
...
$validationCmd=Get-Content -LiteralPath (Join-Path $Root 'RUN_VALIDATION.cmd') -Raw
...
$packageRulesPath=Join-Path $Root 'Knowledge\package-rules.json'
$packageRulesText=if(Test-Path -LiteralPath $packageRulesPath -PathType Leaf){Get-Content -LiteralPath $packageRulesPath -Raw}else{''}
...
# v1.1.1: exact repak extraction must drain both redirected streams
# concurrently and must never wait forever for a single entry.
foreach($marker in @('CopyToAsync','ReadToEndAsync','WaitForExit($timeoutMs)','repak get TIMEOUT','timeoutSeconds = 180')){
  if($pakService -notmatch [regex]::Escape($marker)){Fail "PakService bounded repak-get marker missing: $marker"}
}
if($pakService -match 'BaseStream\.CopyTo\(\$fs\)'){Fail 'PakService reintroduced synchronous stdout CopyTo in repak get.'}
if($pakService -match 'StandardError\.ReadToEnd\(\)'){Fail 'PakService reintroduced synchronous stderr ReadToEnd in repak get.'}

# v1.1.1 final: one append-only smart log. Exact consecutive repeats are
# coalesced with first/last timestamps and periodic checkpoints; unique output
# is not truncated, rotated, or ZIP-compressed away.
foreach($marker in @('$Script:PMMLogRepeatCheckpointSeconds = 5','Write-PMMRepeatSummaryUnsafe','same event x','first={1} | last={2}','Start-PMMLogSession','Stop-PMMLogSession','SESSION START','SESSION END','Write-PMMProcessOutputLog')){
  if($common -notmatch [regex]::Escape($marker)){Fail "Smart-log marker missing: $marker"}
}
foreach($forbidden in @('Rotate-PMMLogIfNeeded','PMMLogMaxBytes','PMMLogArchivesToKeep','PMMLogMessageMaxChars','New-PMMLogZip','Copy-PMMLogTail')){
  if($common -match [regex]::Escape($forbidden)){Fail "Obsolete rotating/truncating logger remains: $forbidden"}
}
if($common -match 'child-process output truncated'){Fail 'Unique child-process diagnostics are still truncated.'}
if($xamlDefault -notmatch [regex]::Escape('support file: Logs\PalModMerger.log')){Fail 'Default Settings UI does not identify the single support log path.'}
if($xamlEn -notmatch [regex]::Escape('support file: Logs\PalModMerger.log')){Fail 'English Settings UI does not identify the single support log path.'}
if($xamlEs -notmatch [regex]::Escape('archivo para soporte: Logs\PalModMerger.log')){Fail 'Spanish Settings UI does not identify the single support log path.'}
if($pakService -notmatch '\$logSuccessfulCommand=\(\$verb -ne ''list''\)'){Fail 'Successful repak list commands are not suppressed from normal logging.'}
if($pakService -match 'repak get OK after'){Fail 'Redundant fast repak-get success logging is still present.'}
if($pakService -notmatch 'repak get SLOW success'){Fail 'Slow successful repak-get diagnostics are missing.'}
if($pakService -match 'Exact asset-family extraction OK'){Fail 'Redundant exact asset-family success logging is still present.'}
foreach($prefix in @('PMMCore','AssetReader','AssetReader DataTable')){if($merge -notmatch 'Write-PMMProcessOutputLog'){Fail "Child-process diagnostic logging missing for $prefix."}}
...
if($start -notmatch 'PMM\.exe'){Fail 'Start.cmd does not launch PMM.exe.'}
if($isReleaseBuilder){
  ...
  if($start -match 'SmokeTest\.ps1'){Fail 'Public Start.cmd must not execute the maintainer SmokeTest.'}
  if($validationCmd -notmatch 'SmokeTest\.ps1'){Fail 'RUN_VALIDATION.cmd does not execute SmokeTest.ps1.'}
}
...
foreach($marker in @('Get-PMMPlanSchemaVersion { return 15 }','Get-PMMEngineId','DeploymentSuppressions')){if($merge -notmatch [regex]::Escape($marker)){Fail "RC34 merge-plan contract marker missing: $marker"}}
if($packageRulesText -notmatch 'PMM_PACKAGE_RULES_V1' -or $packageRulesText -notmatch 'nexus-palworld-4935-ribbuny-pelt-v1'){Fail 'Package-choice Knowledge rule is missing.'}
foreach($marker in @('Get-PMMPackageChoiceAnalyses','PackageChoicePendingReanalysis',"Mode='PackageChoice'")){if($merge -notmatch [regex]::Escape($marker)){Fail "Package-choice merge marker missing: $marker"}}
if($library -notmatch 'Get-PMMPackageChoiceDeploymentSuppressions'){Fail 'Package-choice deployment suppression is missing.'}
...
if($xamlEn -notmatch 'Palworld Manager Merger v1.2 RC1' -or $xamlEs -notmatch 'Palworld Manager Merger v1.2 RC1'){Fail 'v1.2 RC1 window branding is missing.'}
...
# v1.2 RC1 Host/Runtime routing contract.
$hostSource=Get-Content -LiteralPath (Join-Path $Root 'Host\main.go') -Raw
$runnerSource=Get-Content -LiteralPath (Join-Path $Root 'Runner\PMM-Runner.ps1') -Raw
if($hostSource -notmatch 'PMM_AI_HANDOFF_EMERGENCY_V1'){Fail 'PMM Host emergency AI handoff contract missing.'}
if($hostSource -notmatch 'security status' -or $hostSource -notmatch 'doctor'){Fail 'PMM Host doctor/security commands missing.'}
if($runnerSource -notmatch 'Operations\\' -or $runnerSource -notmatch 'Unknown operation'){Fail 'PMM Runner dynamic operation discovery missing.'}
if($start -notmatch 'PMM\.exe'){Fail 'Public Start.cmd does not route through PMM.exe.'}
if($start -match 'Setup-Dependencies\.ps1'){Fail 'Public Start.cmd still owns dependency execution instead of PMM Host/Runner.'}

$runtimeSource=Get-Content -LiteralPath (Join-Path $Root 'Runtime\main.go') -Raw
$runtimeDeps=Get-Content -LiteralPath (Join-Path $Root 'Runtime\deps.go') -Raw
$runtimeContract=Get-Content -LiteralPath (Join-Path $Root 'Runtime\runtime-contract.json') -Raw
$runnerStart=Get-Content -LiteralPath (Join-Path $Root 'Runner\Operations\start.ps1') -Raw
foreach($marker in @('dependencies ensure','knowledge validate','process run','PMMRUNTIME_SELFTEST_OK')){if(($runtimeSource+$runtimeDeps) -notmatch [regex]::Escape($marker)){Fail "PMMRuntime marker missing: $marker"}}
if($runnerStart -notmatch 'PMMRuntime\.exe' -or $runnerStart -notmatch 'dependencies ensure' -or $runnerStart -notmatch '& $runtime ui'){Fail 'Runner start operation does not delegate startup to PMMRuntime.'}
if($setup -notmatch 'PMMRuntime\.exe' -or $setupCode -match 'New-Object System\.Collections\.Generic|Add-Type|\[pscustomobject\]'){Fail 'Setup-Dependencies.ps1 is not a CLM-safe PMMRuntime wrapper.'}
if($runtimeContract -notmatch '"requires_powershell_full_language"\s*:\s*false'){Fail 'Runtime contract does not declare FullLanguage independence.'}

# RC1 native start routing contract.
$routesPath=Join-Path $Root 'Runner\routes.json'
try{$routes=Get-Content -LiteralPath $routesPath -Raw|ConvertFrom-Json}catch{Fail ('Runner/routes.json is invalid: '+$_.Exception.Message);$routes=$null}
if($routes -and [string]$routes.schema -ne 'PMM_HOST_ROUTES_V1'){Fail 'Runner/routes.json schema mismatch.'}
if($routes -and (-not$routes.routes.start -or [string]$routes.routes.start.kind -ne 'native' -or [string]$routes.routes.start.executable -ne 'PMMRuntime.exe')){Fail 'RC1 start route is not native PMMRuntime.'}
$nativeUi=Get-Content -LiteralPath (Join-Path $Root 'Runtime\native_shell_windows.go') -Raw
if($nativeUi -notmatch 'CreateWindowExW' -or $nativeUi -notmatch 'launchNativeUIShell'){Fail 'Native Win32 UI source contract missing.'}

# Clean v1.2 RC1 release contract.
$buildId=(Get-Content -LiteralPath (Join-Path $Root 'BUILD_ID.txt') -Raw).Trim()
if($buildId -ne 'PMM-v1.2-RC1'){Fail "Unexpected BUILD_ID.txt: $buildId"}
...
if($failures.Count -gt 0){
  Write-ValidationLog 'END' ("FAILED with {0} issue(s)." -f $failures.Count)
  Write-Host 'Palworld Manager Merger v1.2 RC1 smoke test FAILED:' -ForegroundColor Red
  foreach($f in $failures){Write-Host " - $f"}
  Write-Host ("Validation log: {0}" -f $validationLogPath)
  exit 1
}
Note 'Source/XAML/runtime-contract checks passed.'
Write-ValidationLog 'END' 'PASSED.'
exit 0
